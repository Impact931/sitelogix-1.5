...
export interface PayrollEntry {
  ...
  work_location: 'on-site' | 'off-site';
  hourly_rate: number;
  total_cost: number;
...
export interface DailyPayrollReport {
  date: string;
...
export interface ProjectLaborCosts {
  projectId: string;
  projectName: string;
  startDate: string;
  endDate: string;
 ...
 */
function toCSVRow(entry: PayrollEntry): string {
  const fields = [
    entry.report_date,
 ...
 */
function validatePayrollEntry(data: CreatePayrollEntryInput): void {
  const errors: string[] = [];

  // Required fields
  if (!data.report_id) errors.push('report_id is required');
  if (!data.report_date) errors.push('report_date is required');
  if (!data.project_id) errors.push('project_id is required');
  if (!data.project_name) errors.push('project_name is required');
  if (!data.employee_id) errors.push('employee_id is required');
  if (!data.employee_name) errors.push('employee_name is required');
  if (!data.employee_number) errors.push('employee_number is required');
  if (!data.work_location) errors.push('work_location is required');
  ...
  if (data.regular_hours < 0) errors.push('regular_hours must be non-negative');
  if (data.overtime_hours < 0) errors.push('overtime_hours must be non-negative');
  if (data.double_time_hours < 0) errors.push('double_time_hours must be non-negative');

  // Validate hourly rate
  if (data.hourly_rate <= 0) errors.push('hourly_rate must be greater than 0');
 ...
 */
function generateSortKey(employeeId: string, timestamp?: number): string {
...
class PayrollService {
  ...
  private readonly tableName = 'sitelogix-payroll-entries';
  ...
  async createBulkEntries(
    entries: CreatePayrollEntryInput[]
  ): Promise<PayrollEntry[]> {
    ...
    const createdEntries: PayrollEntry[] = [];
   ...
   */
  async getEntryById(entryId: string): Promise<PayrollEntry | null> {
    ...
    try {
      ...
      return response.Items[0] as PayrollEntry;
    } catch (error) {
   ...
   */
  async getEntriesByReport(reportId: string): Promise<PayrollEntry[]> {
    ...
    try {
      const response = await this.docClient.send(command);
      return (response.Items || []) as PayrollEntry[];
    } catch (error) {
   ...
   */
  async getEntriesByEmployee(
    employeeId: string,
    startDate: string,
    endDate: string
  ): Promise<PayrollEntry[]> {
    console.log(
      `🔍 Getting entries for employee ${employeeId} from ${startDate} to ${endDate}`
    );
    ...
    const command = new QueryCommand({
      ...
      KeyConditionExpression:
        'employee_id = :employeeId AND report_date BETWEEN :startDate AND :endDate',
      ExpressionAttributeValues: {
  ...
  async getEntriesByProject(
    ...
    console.log(
      `🔍 Getting entries for project ${projectId} from ${startDate} to ${endDate}`
    );
    ...
    const command = new QueryCommand({
      ...
      KeyConditionExpression:
        'project_id = :projectId AND report_date BETWEEN :startDate AND :endDate',
      ExpressionAttributeValues: {
        ':projectId': projectId,
        ':startDate': formatDate(startDate),
        ':endDate': formatDate(endDate),
   ...
   * Uses GSI3-DateIndex
   */
  async getEntriesByDate(date: string): Promise<PayrollEntry[]> {
  ...
  async updatePayrollEntry(
    ...
    const hoursOrRateChanged =
      data.regular_hours !== undefined ||
    ...
    Object.keys(data).forEach((key) => {
      if (key !== 'entry_id' && key !== 'PK' && key !== 'SK') {
        ...
        attributeValues[`:${key}`] = data[key as keyof PayrollEntry];
      }
    });
    ...
    try {
      ...
      return response.Attributes as PayrollEntry;
    } catch (error) {
   ...
   */
  async markAsReviewed(entryId: string): Promise<void> {
   ...
   */
  async deletePayrollEntry(entryId: string): Promise<void> {
   ...
   */
  async generateDailyPayrollReport(
    date: string
  ): Promise<DailyPayrollReport> {
    ...
    const report: DailyPayrollReport = {
      date: formatDate(date),
    ...
    console.log(
      `✅ Generated report: ${report.totalEmployees} employees, ${report.totalHours} hours, $${report.totalCost}`
    );
  ...
  async getEmployeeTimesheet(
    ...
    console.log(
      `📊 Generating timesheet for employee ${employeeId} from ${startDate} to ${endDate}`
    );

    // Get all entries for employee in date range using GSI1
    const entries = await this.getEntriesByEmployee(
      employeeId,
      startDate,
      endDate
    );
    ...
    console.log(
      `✅ Timesheet generated: ${timesheet.totalHours} hours, $${timesheet.totalCost}`
    );
   ...
   */
  async getProjectLaborCosts(
    projectId: string,
    startDate: string,
    endDate: string
  ): Promise<ProjectLaborCosts> {
    console.log(
      `📊 Generating labor costs for project ${projectId} from ${startDate} to ${endDate}`
    );

    // Get all entries for project in date range using GSI2
    const entries = await this.getEntriesByProject(
      projectId,
      startDate,
      endDate
    );
    ...
    const laborCosts: ProjectLaborCosts = {
      projectId,
    ...
    console.log(
      `✅ Labor costs calculated: ${laborCosts.uniqueEmployees} employees, ${laborCosts.totalHours} hours, $${laborCosts.totalCost}`
    );
   ...
   */
  async exportDailyPayrollCSV(date: string): Promise<string> {
   ...
   */
  async exportEmployeeTimesheetCSV(
    employeeId: string,
    startDate: string,
    endDate: string
  ): Promise<string> {
    console.log(
      `📄 Exporting employee timesheet CSV for ${employeeId} from ${startDate} to ${endDate}`
    );

    const timesheet = await this.getEmployeeTimesheet(
      employeeId,
      startDate,
      endDate
    );
   ...
   */
  async exportProjectLaborCostsCSV(
    projectId: string,
    startDate: string,
    endDate: string
  ): Promise<string> {
    console.log(
      `📄 Exporting project labor costs CSV for ${projectId} from ${startDate} to ${endDate}`
    );

    const laborCosts = await this.getProjectLaborCosts(
      projectId,
      startDate,
      endDate
    );
    ...
    const csv = csvRows.join('\n');
    console.log(
      `✅ Project labor costs CSV exported: ${laborCosts.dailyBreakdown.length} days`
    );
