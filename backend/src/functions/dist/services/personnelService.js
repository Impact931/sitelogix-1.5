 ...
 */
function generateAliasKey(alias) {
...
class PersonnelService {
    ...
    async searchEmployeesByName(name) {
        ...
        try {
            ...
            const matches = result.Items.filter((item) => {
                ...
                const aliasMatch = employee.knownAliases.some((alias) => alias.toLowerCase().includes(normalizedSearch));
    ...
    async listEmployees(filters) {
        try {
            ...
            if (filters?.status) {
                ...
                result = await this.docClient.send(new lib_dynamodb_1.QueryCommand({
                    TableName: this.tableName,
                    IndexName: 'GSI3-StatusIndex',
                    KeyConditionExpression: 'employment_status = :status',
                    ExpressionAttributeValues: {
                ...
                result = await this.docClient.send(new lib_dynamodb_1.ScanCommand({
                    TableName: this.tableName,
            ...
            if (filters?.needsProfileCompletion !== undefined) {
                employees = employees.filter((emp) => emp.needsProfileCompletion === filters.needsProfileCompletion);
            }
     ...
     */
    async updateEmployee(personId, data) {
     ...
     */
    async addEmployeeAlias(personId, alias) {
     ...
     * @param alias - Alias string
     */
    async writeAliasRecord(personId, alias) {
     ...
     * @param personId - Person ID
     */
    async terminateEmployee(personId) {
        ...
        await this.docClient.send(new lib_dynamodb_1.UpdateCommand({
            ...
            UpdateExpression: 'SET employmentStatus = :status, employment_status = :status, updatedAt = :updatedAt',
            ExpressionAttributeValues: {
                ':status': 'terminated',
                ':updatedAt': new Date().toISOString(),
            },
     ...
     */
    async matchOrCreateEmployee(name, context) {
        ...
        if (fuzzyMatches.length === 1) {
            ...
            console.log(`✅ LAYER 3: Fuzzy match found - ${match.employee.fullName} (confidence: ${match.similarity.toFixed(1)}%)`);
            return {
     ...
     */
    async exactNameMatch(normalizedName) {
        try {
            const result = await this.docClient.send(new lib_dynamodb_1.QueryCommand({
                TableName: this.tableName,
     ...
     */
    async aliasSearch(normalizedName) {
        try {
     ...
     */
    async fuzzyNameMatch(normalizedName) {
        try {
            ...
            for (const item of result.Items) {
                ...
                const distance = levenshteinDistance(normalizedName.toLowerCase(), employee.fullName.toLowerCase());
     ...
     */
    async contextBasedMatch(candidates, projectId) {
        // Filter candidates who recently worked on the same project
        const projectMatches = candidates.filter((c) => c.employee.lastSeenProjectId === projectId);
     ...
     */
    async createAutoEmployee(normalizedName, parsedName, context) {
        const personId = `PER#${generateEmployeeNumber()}`;
        const now = new Date().toISOString();
        const employee = {
            personId,
     ...
     */
    async suggestMerge(personId1, personId2) {
        ...
        // Aliases to merge
        const aliasesToMerge = duplicate.knownAliases.filter((alias) => !primary.knownAliases.includes(alias));
     ...
     */
    async mergeEmployees(primaryId, duplicateId) {
        ...
        if (!primary.hireDate && duplicate.hireDate)
            updates.hireDate = duplicate.hireDate;
        if (!primary.hourlyRate && duplicate.hourlyRate)
            updates.hourlyRate = duplicate.hourlyRate;
...
// EXPORT SINGLETON
// ============================================================================
exports.personnelService = new PersonnelService();
//# sourceMappingURL=personnelService.js.map