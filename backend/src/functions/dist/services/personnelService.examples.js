...
exports.processVoiceReport = processVoiceReport;
exports.createCompleteEmployeeProfile = createCompleteEmployeeProfile;
exports.handleNicknamesAndAliases = handleNicknamesAndAliases;
exports.demonstrateFuzzyMatching = demonstrateFuzzyMatching;
exports.detectAndResolveDuplicates = detectAndResolveDuplicates;
exports.searchAndFilterEmployees = searchAndFilterEmployees;
exports.updateEmployeeInformation = updateEmployeeInformation;
exports.employeeLifecycle = employeeLifecycle;
exports.runAllExamples = runAllExamples;
const personnelService_1 = require("./personnelService");
// ============================================================================
...
async function processVoiceReport() {
  ...
  `;
    ...
    for (const name of employeeNames) {
        const result = await personnelService_1.personnelService.matchOrCreateEmployee(name, context);
        ...
        if (result.suggestedMatches && result.suggestedMatches.length > 0) {
            ...
            result.suggestedMatches.forEach((match) => {
                console.log(`    • ${match.name} (${match.confidence.toFixed(1)}% - ${match.reason})`);
            });
        }
        console.log('');
    }
...
async function createCompleteEmployeeProfile() {
    ...
    const employee = await personnelService_1.personnelService.createEmployee({
        firstName: 'Robert',
        lastName: 'Johnson',
        middleName: 'James',
        preferredName: 'Bob',
...
async function handleNicknamesAndAliases() {
    ...
    for (const nickname of nicknames) {
        await personnelService_1.personnelService.addEmployeeAlias(employee.personId, nickname);
    ...
    for (const variation of variations) {
        const result = await personnelService_1.personnelService.matchOrCreateEmployee(variation);
        console.log(`  "${variation}" → ${result.confidence} match (${result.matchMethod})`);
    }
    console.log('');
}
// ============================================================================
...
async function demonstrateFuzzyMatching() {
    ...
    for (const typo of typos) {
        const result = await personnelService_1.personnelService.matchOrCreateEmployee(typo);
        const matched = result.employeeId === employee.personId;
        console.log(`  "${typo}" → ${matched ? '✅ MATCHED' : '❌ NOT MATCHED'} (${result.confidence})`);
    }
    console.log('');
}
// ============================================================================
...
async function detectAndResolveDuplicates() {
    ...
    const emp1 = await personnelService_1.personnelService.createEmployee({
        firstName: 'William',
        lastName: 'Thompson',
        email: 'bill.thompson@example.com',
        hourlyRate: 42.0,
    });
    const emp2 = await personnelService_1.personnelService.createEmployee({
        firstName: 'Bill',
        lastName: 'Thompson',
        phone: '555-7890',
        jobTitle: 'Carpenter',
    ...
    // Preview the merge
    const preview = await personnelService_1.personnelService.suggestMerge(emp1.personId, emp2.personId);
    ...
    // Perform the merge
    const merged = await personnelService_1.personnelService.mergeEmployees(emp1.personId, emp2.personId);
    console.log('\nMerge Complete:');
...
async function searchAndFilterEmployees() {
    ...
    await personnelService_1.personnelService.createEmployee({
        firstName: 'John',
        lastName: 'Smith',
        employmentStatus: 'active',
    });
    ...
    const smiths = await personnelService_1.personnelService.searchEmployeesByName('Smith');
    smiths.forEach((emp) => {
        console.log(`  • ${emp.fullName} (${emp.employmentStatus}) - ${emp.personId}`);
    });
    // List all active employees
    console.log('\nAll Active Employees:');
    const activeEmployees = await personnelService_1.personnelService.listEmployees({ status: 'active' });
    ...
    const incompleteProfiles = await personnelService_1.personnelService.listEmployees({
...
async function updateEmployeeInformation() {
    ...
    const updated = await personnelService_1.personnelService.updateEmployee(employee.personId, {
...
async function employeeLifecycle() {
    ...
    const voiceMatch = await personnelService_1.personnelService.matchOrCreateEmployee('Tom Brady', {
    ...
    const completed = await personnelService_1.personnelService.updateEmployee(voiceMatch.employeeId, {
    ...
    const promoted = await personnelService_1.personnelService.updateEmployee(voiceMatch.employeeId, {
    ...
    await personnelService_1.personnelService.terminateEmployee(voiceMatch.employeeId);
    const terminated = await personnelService_1.personnelService.getEmployeeById(voiceMatch.employeeId);
...
//# sourceMappingURL=personnelService.examples.js.map